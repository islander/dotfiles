...
[ -f ~/.npm.bash ] && source ~/.npm.bash

# Kubernetes completions
[ -f ~/.k8s.bash ] && source ~/.k8s.bash
...
export GPGKEY=FACA5805D0F16FCD

export VAGRANT_DEFAULT_PROVIDER=libvirt

ova2qcow() {
  image=$1
  if file "$image" | grep "tar archive"; then
    vmdk="$(tar -tf $image | grep vmdk)"
    if [[ "$vmdk" != "" ]]
    then
      tar xfv $image
      echo converting $vmdk to ${image%.ova}.qcow2
      qemu-img convert -O qcow2 $vmdk ${image%.ova}.qcow2
      echo "Done!"
    else
